...
*
*   pcasl3d11    4/12/18	updates for better BGS using saturation pulses
*				shrinking the rf repeat for better labeling
*				including cardiac triggering
*/
/*@End*************************************************************/
...
float 	B1tag_distance;   /*distance between center of slab and tagging plane*/
...
int 	isRFon=0;           /* variable used to toggle the RF */
...
int 	isOdd = 0;
int	doZgrappa = 0;
int	doASLcardiacTrig = 0;   /* enable cardiac triggering before the pre label saturation */

float 	fractional_moment=0.9;  /* 1 = zeroth moment of labeling gradient will be 0 */
/* int 	t_rfrepeat = 2000;   pseudo-continuous labeling RF pulse spacing */
/* int t_rfrepeat = 1500; */  /* pseudo-continuous labeling RF pulse spacing */
int t_rfrepeat = 1000;      /* lhg 4/12/18 pseudo-continuous labeling RF pulse spacing */
int 	tmin_rfrepeat;
int 	nreps;  /* number of times the RF labeling pulses need to be played */
float 	pcasl_flip = 25;  /* flip angle of each Hanning pulse */
/*int  	myramptime = 120;*/ /* lhg 9/21/12   ramp times for the pcasl pulses */
int  	myramptime = 60; /* lhg 4/12/18   ramp times for the pcasl pulses */
double 	pcasl_Gmax = 0.6; /* default gradient max aplitude in pcasl pulse train */
/* double 	pcasl_Gave = 0.03;*/ /* default average gradient momoent */
double 	pcasl_Gave = 0.06; /* lhg 4/12/18 default average gradient momoent */

double 	area_gzrftag1;
double 	area_gzrftag2;
double 	area_gzrftag1c;
double 	area_gzrftag2c;
double 	area_gzrftag1r;
double 	area_gzrftag2r;
double 	area_gzrftag1rc;
double 	area_gzrftag2rc;
double 	myphase_increment = 0;
double 	my_radians_per_CM;
double 	my_notch_distance;
int 	closest_notch;
double  manual_phase_correction = 0;

/* LHG 12/14/08  add some variables for RT changes*/
int 	tdelaycore_fix = 11;
double 	myphase_increment_0 = 0;
double 	myPhaseCorr = 0;
int 	doRT=0;
int 	ia_gzrftag1r_0;   /* initial value for refocuser's amplitude in pCASL pulses - LHG 6/30/09 */
...
int	BS1_time = 200000; /* first delay between BS inversion pulses , default to 300 ms after tag*/
int 	BS2_time = 1200000;  /* delay between second BS inversion pulse and imaging, default to 850 ms after the first pulse */
...
abstract("Real Time, interactive, Pseudo CASL with 3D, Spiral acquisition for fMRI sequence with GRAPPA-Z, optimized");
psdname("pcasl3d11.e");
...
int cveval()
{
	...
	cvdesc(opuser16, "Optimize Phase Correction ? (will search in steps of 0.2 rads)");
	cvdef(opuser16, 0);
	opuser16 = 0;
	cvmin(opuser16, 0.);
	cvmax(opuser16, 1.);
	multiphsFlag = opuser16;
	...
	/* -------------------------------------------------------------
	   ...
	   Define CVs for duration, amplitude and location of the spin
	   tagging pulses
	   -------------------------------------------------------------*/
	cvdesc(opuser20,"Duration of the tagging period (ms)");
	cvmin(opuser20,0);
	cvmax(opuser20,8000);
	cvdef(opuser20,3000);
	opuser20 = 1800;
	...
	cvdesc(opuser22,"Dist. from landmark to tagging plane (cm)");
	cvmin(opuser22,-50);
	cvmax(opuser22, 50);
	cvdef(opuser22,8);
	opuser22 = 10;
	...
	/*  LHG 9/8/14	
	    cvdesc(opuser23,"Double Coil Fequency Offset (Hz)");
	    cvmin(opuser23,0);
	    cvmax(opuser23, 15000);
	    cvdef(opuser23,10000);
	    opuser23 = 10000;
	 */
	cvdesc(opuser23,"Manual pCASL phase correction (rads)");
	cvmin(opuser23,-2*PI);
	cvmax(opuser23, 2*PI);
	cvdef(opuser23,0.0);
	opuser23 = 0.0;
	...
	cvdesc(opuser25,"Control Type (0= Toggle RF, 1= Flip Grads)");
	cvmin(opuser25,0);
	cvmax(opuser25, 1);
	cvdef(opuser25,1);
	opuser25 = 1;

	cvdesc(opuser26,"SE Crusher Type (1= Straddle 180  else  bipolar)");
	cvmin(opuser26,0);
	cvmax(opuser26, 1);
	cvdef(opuser26,0);
	opuser26 = 0;

	t_tag = RUP_GRD(opuser20*1000);
	t_delay = RUP_GRD(opuser21 * 1000);
	B1tag_distance = opuser22-(float)piscancenter/10;
	mycrush=0;
	doZgrappa = opuser24;
	mycontrol=opuser25;
	crushtype=opuser26;
	/* LHG 9/8/14 */
	manual_phase_correction = opuser23;

	/* PCASL inversion pulse definitions */
	/* LHG 11/29/12 
	   Determining the amplitude of the pcasl pulse 

	   Better YET:  use the actual prescan pulse:  a Hanning windowed sinc
	   when a_rf2mps=1, that means you get a 180

	   t=linspace(-1600,1600,3200)';
	   s1=sinc(t/800).*hanning(3200);
	   plot(s1)
	   area_sinc=sum(abs(s1(:)));  % =====> 891.37

	   h1 = hanning(500);
	   area_hanning = sum(abs(h1(:)));

	   rfscale180 = area_sinc/ area_hanning
	   ======>  3.5592 
	 */

	pw_rftag1 = 500;     
	/*pw_rftag1 = 1000; */  /* increased from 500  - LHG 12/7/12 */  

	a_rftag1 = (3.5592)*(pcasl_flip/180) * 500/pw_rftag1;
	a_rftag2 = a_rftag1;
	a_rftag1c = a_rftag1;
	a_rftag2c = -a_rftag1;

	pw_rftag2 = pw_rftag1;
	pw_rftag1c = pw_rftag1;  
	pw_rftag2c = pw_rftag1;

	res_rftag1 = 250;
	res_rftag2 = res_rftag1;
	res_rftag1c = res_rftag1;
	res_rftag2c = res_rftag1;

	a_gzrftag1 = pcasl_Gmax; /* G/cm */
	a_gzrftag2 = a_gzrftag1;
	a_gzrftag1c = a_gzrftag1;
	a_gzrftag2c = a_gzrftag1;

	pw_gzrftag1 = pw_rftag1;
	pw_gzrftag1a = myramptime;
	pw_gzrftag1d = pw_gzrftag1a;

	pw_gzrftag1c = pw_rftag1;
	pw_gzrftag1ca = pw_gzrftag1a;
	pw_gzrftag1cd = pw_gzrftag1a;

	pw_gzrftag2 = pw_rftag1;
	pw_gzrftag2a = pw_gzrftag1a;
	pw_gzrftag2d = pw_gzrftag1a;

	pw_gzrftag2c = pw_rftag1;
	pw_gzrftag2ca = pw_gzrftag1a;
	pw_gzrftag2cd = pw_gzrftag1a;
	/* refocusers:  */
	pw_gzrftag1r = 200;  
	pw_gzrftag1r = 100; /*lhg 4/12/18 */  
	pw_gzrftag1ra = myramptime;  

	pw_gzrftag1rd = pw_gzrftag1ra;

	pw_gzrftag1rc = pw_gzrftag1r;
	pw_gzrftag1rca = pw_gzrftag1ra;
	pw_gzrftag1rcd = pw_gzrftag1ra;

	pw_gzrftag2r = pw_gzrftag1r;
	pw_gzrftag2ra = pw_gzrftag1ra;
	pw_gzrftag2rd = pw_gzrftag1ra;

	pw_gzrftag2rc = pw_gzrftag1r;
	pw_gzrftag2rca = pw_gzrftag1ra;
	pw_gzrftag2rcd = pw_gzrftag1ra;

	/* compute the areas of the slice select pulses */
	area_gzrftag1 =  (double)(a_gzrftag1  * (pw_gzrftag1  + pw_gzrftag1a));
	area_gzrftag2 =  (double)(a_gzrftag2  * (pw_gzrftag2  + pw_gzrftag2a));
	area_gzrftag1c = (double)(a_gzrftag1c * (pw_gzrftag1c + pw_gzrftag1ca));
	area_gzrftag2c = (double)(a_gzrftag2c * (pw_gzrftag2c + pw_gzrftag2ca));

	/* COmpute refocuser gradient strength based on desired G_ave (instead of fractional moment) */
	a_gzrftag1r = (pcasl_Gave*t_rfrepeat - area_gzrftag1)/(pw_gzrftag1ra + pw_gzrftag1r);
	a_gzrftag1rc = a_gzrftag1r;
	a_gzrftag2r = a_gzrftag1r;
	a_gzrftag2rc = a_gzrftag1r;

	/* compute the areas of the slice refocusers */
	area_gzrftag1r =  (double)(a_gzrftag1r  * (pw_gzrftag1r  + pw_gzrftag1ra));
	area_gzrftag2r =  (double)(a_gzrftag2r  * (pw_gzrftag2r  + pw_gzrftag2ra));
	area_gzrftag1rc =  (double)(a_gzrftag1rc  * (pw_gzrftag1rc  + pw_gzrftag1rca));
	area_gzrftag2rc =  (double)(a_gzrftag2rc  * (pw_gzrftag2rc  + pw_gzrftag2rca));

	B1tag_offset = -a_gzrftag1*(B1tag_distance * GAMMA_H1) / (2*PI);

	tmin_rfrepeat = pw_gzrftag2 +
		2*pw_gzrftag2d +
		2*pw_gzrftag2ra +
		pw_gzrftag2r +
		timessi +
	...
	/* LHG new version of phase calculation 8/28/07   */
	/* matlab simulation uses this:
	   RF_theta = gam  * (Gz_ss + Gz_phaser)*1e-4*(length(myHanning)*dt/2) * tag_loc ;
	   recall GAMMA_H1 is in rad/s/G
	   Area is in units of (G/cm * us)
	   The area of the refocuser is defined in pulsegen as the area of the
	   slice select graddient * the fractional_moment
	 */
	/* LHG 10/18/12  - replace calculation by using the areas directly:  
	   myphase_increment = GAMMA_H1 * (double)area_gzrftag1 / (1000000) * (1-fractional_moment) *  B1tag_distance;
	 */
	/* LHG 2017.12.12 use this instead of the one below */
	myphase_increment = -GAMMA_H1 * pcasl_Gave * t_rfrepeat * 1e-6*  B1tag_distance;

	/*myphase_increment = GAMMA_H1 * (double)(area_gzrftag1 + area_gzrftag1r) / (1000000) *  B1tag_distance;*/
	/* (REMOVED on 12/5/12)----> The negative sign is because the tag_distance should be a negative number!  */

	/* 9/8/14 :  phase correction from the user interface */ 
	myphase_increment += manual_phase_correction;

	/* wrap phase to (-pi,pi) range */
	myphase_increment = atan2 (sin(myphase_increment), cos(myphase_increment)); 	

	nreps=(int)(t_tag / (2*t_rfrepeat ) );
	...
	rfscalesech = 0.2153;
	rfscalesech = 0.75;  /* Empirically on sphere phantom, I didn't get an inversion unutl 0.65) */
...
2*pw_gzspoila);
	 */
	astseqtime = (2*t_rfrepeat) ;
	astseqtime2 = nreps*astseqtime + t_delay;  /*duration of tagging + postabeling delay */

	tmin = RUP_GRD(tlead +tdel+
			/* tpre + pw_rf0  +  */
			/* tpre + pw_gz0 + 2*pw_gz0a + */
			tpre + 0.5*pw_gzrf1 + pw_gzrf1a +
			opte + pw_gx + mapdel +
			2*pw_gzphase1 + 4*pw_gzphase1a +
	...
	fprintf(stderr, "\npw_gzphase1=%d , pw_gzphase1a = %d", pw_gzphase1, pw_gzphase1a);
	fprintf(stderr, "\npsdseqtime, seqtr, tmin, optr=  %d   %d  %d  %d\n", psdseqtime,  seqtr,tmin,optr);

	t_adjust = RUP_GRD(optr - psdseqtime*opslquant - t_tag - t_delay - tpre - t_preBS);                 /* lhg: 4/12/18  */
	fprintf(stderr, "t_adjust, astseqtime=  %d  %d \n", t_adjust, astseqtime);
	/*---------------------------*/
...
so don't change the duration of pw_rf1 without adjusting this calculation ...*/
	...
	if(mycrush==1){
		...
		minte += 2*pw_gzsp3 + 4*pw_gzsp3a + delta1;
...
STATUS pulsegen(void)
{
	...
	TRAPEZOID(ZGRAD,
			gzrf1,
			/*RUP_GRD(pendall(&gz0,0) + pw_gzrf1a)  ,*/
	...
	if(mycrush==1)  /* Now put in the flow crushers for the GRE case */
	{
		...
		TRAPEZOID(XGRAD,
				gxsp3,
				RUP_GRD(pend(&gzrf1rd,"gzrf1rd",0)+pw_gxsp3a ),
				0,0,loggrd);
		TRAPEZOID(XGRAD,
				gxsp4,
		...
		TRAPEZOID(YGRAD,
				gysp3,
				RUP_GRD(pend(&gzrf1rd,"gzrf1rd",0)+pw_gysp3a ),
				0,0,loggrd);
		TRAPEZOID(YGRAD,
				gysp4,
		...
		TRAPEZOID(ZGRAD,
				gzsp3,
				RUP_GRD(pend(&gzrf1rd,"gzrf1rd",0)+pw_gzsp3a ),
				0,0,loggrd);
		TRAPEZOID(ZGRAD,
				gzsp4,
	...
	/* lhg: 4/12/18 */
 	/* JS 3/14/18:  Background suppression pulses before label train: adiabatic 90 saturation pulse + spoiler gradient  */
	...
	/* Here are the tagging pulse */

	/* LHG 9.29.12 : PCASL pulses ------------------------------------
	   this imports the Hanning window pulses  - generated using matlab and xlatebin... ?
	   the time to get the RF channel ready is 'psd_rf_wait'
	 */
	fprintf(stderr, "\npulsegen: generating PCASL pulses ... ");
	EXTWAVE(RHO,
			rftag1,
			RUP_GRD(pw_gzrftag1a + psd_rf_wait),
			pw_gzrftag1,
			a_rftag1,
			res_rftag1,
			/usr/g/bin/myhanning.rho,,
			loggrd);
	fprintf(stderr, "\n ... rftag1 done ." );

	TRAPEZOID(ZGRAD,
			gzrftag1,
			RUP_GRD(pw_gzrftag1a),
			0,
			0, loggrd);
	fprintf(stderr, "\n ... gzrftag1 done ." );
	...
	TRAPEZOID(ZGRAD,
			gzrftag1r,
			RUP_GRD(pendall(&gzrftag1,0) + pw_gzrftag1ra),
			0,
			0, loggrd);	
	fprintf(stderr, "\n ... gzrftag1r done ." );
	...
	EXTWAVE(RHO,
			rftag2,
			RUP_GRD(t_rfrepeat + pw_gzrftag2a + psd_rf_wait),
			pw_gzrftag2,
			a_rftag2,
			res_rftag2,
			/usr/g/bin/myhanning.rho,
	...
	TRAPEZOID(ZGRAD,
			gzrftag2,
			RUP_GRD(t_rfrepeat + pw_gzrftag2a),
			0,
			0, loggrd);
	fprintf(stderr, "\n ... gzrftag2 done ." );
	...
	TRAPEZOID(ZGRAD,
			gzrftag2r,
			RUP_GRD(pendall(&gzrftag2,0) + pw_gzrftag2ra),
			0,
			0, loggrd);
	fprintf(stderr, "\n ... gzrftag2r done ." );

	fprintf(stderr, "\nastseqtime=  %d , textra_astcore= %d , sum of gradtimes=%d, beg. of second refocuser=%d , ends at= %d"
			, astseqtime, textra_astcore, 4*pw_gzrftag1a+pw_gzrftag1+pw_gzrftag1r,
			RUP_GRD(pbegall(&gzrftag2r,0)), RUP_GRD(pendall(&gzrftag2r,0)));

	SEQLENGTH(astcore,astseqtime + textra_astcore - timessi,astcore);
	getperiod(&deadtime_astcore, &astcore, 0);


	/* Here are the control pulses :  */	
	fprintf(stderr, "\npulsegen: Now the control PCASL pulses ... ");
	EXTWAVE(RHO,
			rftag1c,
			RUP_GRD(pw_gzrftag1ca + psd_rf_wait),
			pw_gzrftag1c,
			a_rftag1c,
			res_rftag1c,
			/usr/g/bin/myhanning.rho,,
	...
	TRAPEZOID(ZGRAD,
			gzrftag1rc,
			RUP_GRD(pendall(&gzrftag1c,0) + pw_gzrftag1rca),
			0,
			0, loggrd);
	fprintf(stderr, "\n ... gzrftag1rc done ." );
	...
	EXTWAVE(RHO,
			rftag2c,
			RUP_GRD(t_rfrepeat + pw_gzrftag2ca + psd_rf_wait),
			pw_gzrftag1c,
			a_rftag2c,
			res_rftag2c,
			/usr/g/bin/myhanning.rho,
	...
	TRAPEZOID(ZGRAD,
			gzrftag2c,
			RUP_GRD(t_rfrepeat + pw_gzrftag2ca ),
			0,
			0, loggrd);
	fprintf(stderr, "\n ... gzrftag2c done ." );
	...
	TRAPEZOID(ZGRAD,
			gzrftag2rc,
			RUP_GRD(pendall(&gzrftag2c,0) + pw_gzrftag2rca),
			0,
			0, loggrd);
	fprintf(stderr, "\n ... gzrftag2rc done ." );


	SEQLENGTH(controlcore,astseqtime + textra_astcore - timessi,controlcore);
	getperiod(&deadtime_controlcore, &controlcore, 0);
	fprintf(stderr, " \n... all PCASL pulses done. ");
	...
	/*SEQLENGTH(tadjustcore, RUP_RF(optr - t_preBS - t_tag - t_delay - psdseqtime *opslquant + textra_tadjust  ) ,tadjustcore);*/
...
/*---------------------------------------------------------
  ...
  ----------------------------------------------------*/
void doast(int* trig, int isRFon, int* pcasl_iphase);
void rt_updates(void);
void dodelay(int* trig); 
void doadjust(int* trig);

void get_pcasl_phases(int *pcasl_iphase, double myphase_increment, int nreps);
/*---------------------------------------------------------*/
...
#define RESERVED_IPG_MEMORY (0xbfff00)
	...
	rt_t_delay = t_delay;
	rt_TR = optr;
	rt_t_adjust = t_adjust;
	rt_pcasl_flip = pcasl_flip;
	rt_B1tag_distance = B1tag_distance;
	rt_fractional_moment = fractional_moment;
	rt_PhaseCorr = myPhaseCorr;
	myphase_increment_0 = myphase_increment;
	ia_gzrftag1r_0 = ia_gzrftag1r;

	printf("\ncalculated  a_rf1= %f , opflip= %f: , ia_rf1=  %d", a_rf1, opflip, ia_rf1);

	printf("\n TARDIS_FREQ_RES: %d", TARDIS_FREQ_RES);
	printf("\n AST VARIABLES: ");
	printf("\n Gradient max amplitude (a_gzrftag1):   %f", a_gzrftag1);
	printf("\n Gradient Duration  (pw_gzrftag1):  %d", pw_gzrftag1);
	printf("\n Gradient Area (area_gzrftag1):  %d", area_gzrftag1);
	printf("\n Average pcasl gradient: %f" , pcasl_Gave);
	printf("\n Frequency Offset   (B1tag_offset): %d", B1tag_offset);
	printf("\n Tagging Plane distance (B1tag_distance): %f", B1tag_distance);
	printf("\n myphase_increment (rads): %f" , myphase_increment);
	printf("\n myPhaseCorr (rads): %f" , myPhaseCorr);
	printf("\n-------------------\n ");
	fflush(0);
	/* ---------------------------------------------------------------*/
	...
	pcasl_iphase = 	(int *) AllocNode(2*nreps*sizeof(int)); 	/*LHG 12.6.12 */
	...
	if(pre)  {
		...
		for (iv = 0; iv < ndisdaq; iv++) {	/*  disdaqs  */
			...
			/* rt_updates(); */
			doadjust(&trig);
			doast(&trig, isRFon, pcasl_iphase);
			dodelay(&trig);
	...
	/*we don't want the field map when we're adjusting the phase of pcasl pulses*/ 
	if (multiphsFlag) domap=0;
	fprintf(stderr,"\ndomap = %d", domap);
...
void rt_updates()
{
	/*
	   LHG 5/11/09
	   This function will update the phase calculations and gradient calculations for real time mode

LHG:   12/6/12  ----  this function is obsolete ---- needs updating ------ DO NOT USE !

	 */

	/* timing :*/
	nreps=(int)(rt_t_tag / (2*t_rfrepeat ) );
	astseqtime = (2*t_rfrepeat) ;
	astseqtime2 = nreps*astseqtime + rt_t_delay;  /*duration of tagging + postabeling delay */
	rt_t_adjust = RUP_GRD(rt_TR - psdseqtime*opslquant- rt_t_tag - rt_t_delay - tpre);

	/* re-calculate the labeling freq. offset in case it got changed on the fly */
	B1tag_offset = -a_gzrftag1*(rt_B1tag_distance * GAMMA_H1) / (2*PI);

	/* LHG 12/14/08:  make interactive adjustments to the pCASL flip angle */
	rt_ia_rftag1 = (3.5592)*(pcasl_flip/180)*max_pg_iamp;

	setiamp(rt_ia_rftag1, &rftag1, 0);
	setiamp(rt_ia_rftag1, &rftag2, 0);
	setiamp(rt_ia_rftag1, &rftag1c, 0);
	setiamp(-rt_ia_rftag1, &rftag2c, 0);
	/*--------------------------------------*/

	/* LHG 6/25/09:  update the fractional moment for PCASL sequence */
	/*	
		setiampt((int)(ia_gzrftag1r_0*rt_fractional_moment/fractional_moment), &gzrftag1r, 0);
		setiampt((int)(ia_gzrftag1r_0*rt_fractional_moment/fractional_moment), &gzrftag2r, 0);
		setiampt((int)(ia_gzrftag1r_0*rt_fractional_moment/fractional_moment), &gzrftag1rc, 0);
		setiampt((int)(ia_gzrftag1r_0*rt_fractional_moment/fractional_moment), &gzrftag2rc, 0);
	 */

	/* LHG 6.29.09 - update phase increment to adjust for other real time changes
	   in inversion plane location ...etc. */

	/* LHG 10/18/12:  recalculate pcasl pulse phase based on the actual areas instead: 
	   myphase_increment = GAMMA_H1 * (double)area_gzrftag1 / (1000000) * (1-fractional_moment) *  B1tag_distance;	/
	   myphase_increment = GAMMA_H1 * (double)(area_gzrftag1 + area_gzrftag1r) / (1000000) *  B1tag_distance;
	   myphase_increment = -myphase_increment ;
	 */	
	/* LHG 1/29/09:  make interactive adjustments to the pCASL pulses phase correction */
	/*
	   myphase_increment = myphase_increment + rt_PhaseCorr;
	 */
	printf("\nrt_PhaseCorr:  %f",rt_PhaseCorr);
	printf("\nrt_pcasl_flip:  %f",rt_pcasl_flip);
	printf("\nmax_pg_iamp:  %f",max_pg_iamp);
	printf("\nrt_ia_rftag1:  %d",rt_ia_rftag1);
	printf("\nia_gzrftag1r:  %d",ia_gzrftag1r);
	printf("\nmyphase_increment %f", myphase_increment);
	printf("\n-------------------\n ");
...
void doast(int* trig, int isRFon, int* pcasl_iphase)
{
	...
	if (doASLcardiacTrig) settrigger(TRIG_ECG, 0); 
	boffset(off_preBScore);
	startseq(0, MAY_PAUSE);
	if (doASLcardiacTrig) settrigger(TRIG_INTERN, 0); 

	/*------------- LHG: single coil-----
	  setfrequency((int)(B1tag_offset/TARDIS_FREQ_RES), &rftag, 0);
	  setiamp(ia_rftag, &rftag, 0);
	  settrigger(*trig, 0);
	  --------------------------------------*/
	...
	/* LHG 7/11/13:  while collecting the M0 images, we want the label off */
	if (ifr < M0frames){
		setiamp(0,&rftag1,0);
		setiamp(0,&rftag2,0);
		setiamp(0,&rftag1c,0);
	...
	{
		setiamp(ia_rftag1,&rftag1,0);
		setiamp(ia_rftag2,&rftag2,0);
		setiamp(ia_rftag1c,&rftag1c,0);
		setiamp(ia_rftag2c,&rftag2c,0);

	}

	/* Make a table of phases for the pcasl pulses*/
	if (isRFon)
	{
		setfrequency((int)(B1tag_offset/TARDIS_FREQ_RES), &rftag1, 0);
		setfrequency((int)(B1tag_offset/TARDIS_FREQ_RES), &rftag2, 0);
		fprintf(stderr, " Tag ...");
		for (n=0; n<nreps; n++) {
			/*
			   setphase(fmod(pcasl_rfphase,2*PI), &rftag1, 0);
			   setphase(fmod(pcasl_rfphase + myphase_increment,2*PI), &rftag2, 0);
			   pcasl_rfphase = pcasl_rfphase+2*myphase_increment;
			 */			
			/* Quick check to see that we're setting the phase right :
			   getphase(&DABphase, &rftag1,0); 
			   fprintf(stderr," pcasl_rfphase ... %04f OR  %04f", pcasl_rfphase, DABphase);
			 */

			setiphase(pcasl_iphase[n*2], &rftag1, 0);
			setiphase(pcasl_iphase[n*2+1], &rftag2, 0);
	...
	{
		setfrequency((int)(B1tag_offset/TARDIS_FREQ_RES), &rftag1c, 0);
		setfrequency((int)(B1tag_offset/TARDIS_FREQ_RES), &rftag2c, 0);
		fprintf(stderr, " Control ...");

		for (n = 0; n < nreps; n++) {
			/*
			   setphase(fmod(pcasl_rfphase,2*PI), &rftag1c, 0);
			   setphase(fmod(pcasl_rfphase + myphase_increment,2*PI), &rftag2c, 0);
			   pcasl_rfphase = pcasl_rfphase+2*myphase_increment;
			 */
			/* Quick check to see that we're setting the phase right : 
			   getphase(&DABphase, &rftag1c,0); 
			   fprintf(stderr," pcasl_rfphase control ... %04f OR  %04f", pcasl_rfphase, DABphase);
			 */	
			setiphase(pcasl_iphase[n*2], &rftag1c, 0);
			setiphase(pcasl_iphase[n*2+1], &rftag2c, 0);
...
/* LHG 12/6/12 : code to compute the linear phase increment of the PCASL pulses */
void get_pcasl_phases(pcasl_iphase, myphase_increment, nreps)
	int 	pcasl_iphase[];
	double 	myphase_increment;
	int 	nreps;
{
	int 	n;
	double	pcasl_rfphase;

	pcasl_rfphase=0;

	for (n=0; n<nreps*2; n++)
	{
		pcasl_rfphase = atan2 (sin(pcasl_rfphase), cos(pcasl_rfphase));      /* wrap phase to (-pi,pi) range */
		pcasl_iphase[n] = (int)(pcasl_rfphase/M_PI * (float)FS_PI);
		pcasl_rfphase +=myphase_increment;
	}
}
