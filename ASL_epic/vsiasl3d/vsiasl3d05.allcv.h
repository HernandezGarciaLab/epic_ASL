...
#ifndef h_vsiasl3d05_allcv_h
#define h_vsiasl3d05_allcv_h
...
int nl = 1 ; /* number of interleaves */
int scluster = 0 ;
int nextra = 2 ;
int nframes = 1 ;
int total_views = 0;	/*  total shots to gather per slice */
int gating = TRIG_INTERN ;
int psdseqtime = 0;     /* sequence repetition time */

int t_tipdown_core = 0;	/*time it takes to play out the 90 core */
int t_readout_core = 0; 	/* time it takes to play the 180, readout, spoilers until the next 180 */

/* int timessi=400us with {0,,400us,INVIS,"time from eos to ssi in intern trig",};*/
int timessi=120   ;
int trerror = 0;		/* error in TR total */
int nerror = 0;		/* num slices  to take up the TR error */
int gtype = 0 ;
...
int pwgpre = 1000;       /* calculated dephasing width for spiral in-out */
int tadmax = 0;     /* maximum readout duration  */
int tlead = 160;      /* initial lead time for rf pulse prep*/
float daqdeloff = 0.0  ; /* fudge factor, tuned at SRI  */
int daqdel = 128  ; /*  gradient delay vs readout */
int thetdeloff = 0  ;
int espace = 140   ;
int readpos = 0;        /* position of readout gradients, based on oppseq */
int daqpos = 0;        /* position of readout window, based on oppseq */
int minte = 0;
int seqtr = 0 ;
int endtime = 500000 ;
...
int domap = 1 ;
int mapdel = 2000;   /* variable delay for field map */
int bmapnav = 1 ;
...
int 	t_tag = 0;              /*duration of tagging pulse*/
int 	t_delay = 0;            /*delay after tagging before Aquisition*/
int 	B1tag_offset = 0;       /*frequency offset of tagging pulse*/
int 	astseqtime = 0;         /*duration of a single cycle of ast part of the sequence */
int 	astseqtime2 = 0;        /*duration of the total ast part of the sequence */
int 	t_adjust = 0;           /*delay before ast pulse to make up the rest of the TR */
int 	delta1 = 0  ;       /* time between flow spoiling gradients */
...
int	vsi_controlmode = 6  ;  
int	vsi_axis_default = 3  ;
int	vsi_axis = 3 ;
...
float 	vsi_Gmax = 2.0; /* default gradient max aplitude in VSI pulse train */
float 	vsi_Gcontrol = 2.0 ; /* default gradient max aplitude in control pulse train */
...
float	vsi_RFmax = 140 ; /* mGauss. Peak B1 of the pulse train 
				for a 180 deg, 0.4 ms  hard pulse 
				B1max should be 292.50 mG*/ 
float 	rf1_RFmax = 147;   /* the excitation pulse is a hanning windowed 7 lobe sinc
...
float	dummy_RFmax = 147;    /* mGauss.  Peak B1 of reference pulse rfdummy in rf_grad_sprilo.h
...
int 	Npoints = 0;	   /* the number of points that were read from the RF pulse file.  Should match vsi_train_len */
float 	vsi_velocity = 0;   /* velocity of the spins to target for inversion */
int	vsi_timegap = 20000;   /* gap between VSI pulses if we are doing mulitple pulsees */

/* arterial suppression by BIR pulses */
float 	BIR_Gmax = 1.0; /* default gradient max aplitude in VSI pulse train */
...
float 	target = 0;
int 	rtime = 0, ftime = 0;
float 	zfov = 0;   	/* FOV along the z-axis (cm) */
float 	ampGzstep = 0;
float 	areaGzstep = 0;
int 	iampGzstep = 0;
...
float  area_gzrf1r = 0;
float   rf1_bw = 0;
float   slab_fraction=0.95 ;

float	se_slab_fraction = 0.95 ;
...
float	rfscalesech = 0;	/* ratio of areas: autoprescan sinc to SECH pulse */
int	doBS = 1;	/* background suppression pulses */
int	doArtSup = 0;	/*arterial suppression pulses */
int	t_preBS = 0;	/* this is the duration of the core containing the first BS pulse (beofre tagging */
...
float  area_gzBSOrfspoiler = 0;

/* some variables to adjust timing - leftovers from signa */
float 	slwid180 = 1.2 ; /*this is so that the 180 stuff will work */
int	tpre=0;
int	tdel=0;
int	mytpre = 0;
int 	tcs = 0;  /* duration of the chem sat pulses */
...
;
  ...
  float a_gz180crush1 = 0;
  int ia_gz180crush1 = 0;
  int pw_gz180crush1a = 0;
  int pw_gz180crush1d = 0;
  int pw_gz180crush1 = 0;
  int wg_gz180crush1  = 0;
...
;
  ...
  float a_gz180crush2 = 0;
  int ia_gz180crush2 = 0;
  int pw_gz180crush2a = 0;
  int pw_gz180crush2d = 0;
  int pw_gz180crush2 = 0;
  int wg_gz180crush2  = 0;
  ...
  float cyc_rfdummy = 0;
  int off_rfdummy = 0;
  float alpha_rfdummy = 0.46;
  float thk_rfdummy = 0;
  float gscale_rfdummy = 1.0;
  float flip_rfdummy = 0;
  int wg_rfdummy = TYPRHO1 
;
  ...
  int wg_gzBS0rfspoiler  = 0;
	   ...
	   float   a_vsitag1_theta = 0;
	   int    ia_vsitag1_theta = 0;
	   int    pw_vsitag1_theta = 0;
	   int   res_vsitag1_theta = 0;
	   ...
	   float   a_vsictl1_theta = 0;
	   int    ia_vsictl1_theta = 0;
	   int    pw_vsictl1_theta = 0;
	   int   res_vsictl1_theta = 0;
  ...
  int wg_BS2rf_theta  = 0;
	   ...
	   float   a_ASrf_theta = 0;
	   int    ia_ASrf_theta = 0;
	   int    pw_ASrf_theta = 0;
	   int   res_ASrf_theta = 0;
