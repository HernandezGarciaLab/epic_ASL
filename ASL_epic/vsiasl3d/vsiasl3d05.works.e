...
int t_readout_core; 	/* time it takes to play the 180, readout, spoilers until the next 180 */
...
int	vsi_controlmode = 6 with {1,,,VIS, "Control Mode: (1) flip the magnitude  (2) no RF  (3) negative vel target (4) flip grads (5) no VelSel grads, (6) monopolar grads", } ;  
...
int	iTrigType=3 with {0,3, 3, VIS, "TRIG_LINE=0, TRIG_ECG=1,TRIG_AUX=2,TRIG_INTERN=3"};
...
int	doRotated90 = 1 with {0, 3, , VIS, "Forces the 90 degree pulse to the : 1=Y axis, 2=X axis, 3=Z axis "};
...
/* end changes for DV26 */
abstract("vel. sel. ASL with 3D GRASE Spiral acquisition with optional GRAPPAZ");
psdname("vsiasl3d05");
...
int cveval()
{
	...
	*/
	...
	t_adjust = RUP_GRD(optr - seqtr - t_tag - t_delay - t_preBS);                 /* LHG 7.18.18 */
...
so don't change the duration of pw_rf1 without adjusting this calculation ...*/
	...
	*/
	...
	/*pitscan = (nextra + nframes*nl)*(t_adjust + astseqtime + t_tipdown_core + t_readout_core*opslquant);*/
	...
	minte += pw_gzphase1 + 2*pw_gzphase1d;
	/*-------------------------------------------------*/
...
STATUS pulsegen(void)
{
	...
	SEQLENGTH(tipdown_core, t_tipdown_core - timessi + textra , tipdown_core);
	...
	TRAPEZOID(ZGRAD,
			gz180crush1,
			RUP_GRD(tlead + pw_gz180crush1a + psd_rf_wait), 
			0,
			0,loggrd);
	...
	SLICESELZ(rf2, 
			pend(&gz180crush1d, "gz180crush1d",0) + pw_gzrf2a,
			pwrf1, /* use the same pulse as the 90, but double it */ 
			zfov * 10 * se_slab_fraction,  /* (mm) */
			180, /*flip angle should be a 180*/
			cycrf1,, 
			loggrd);
	...
	TRAPEZOID(ZGRAD,
			gzphase1,
			RUP_GRD(pend(&gz180crush2d, "gz180crush2d",0) + pw_gzphase1a),
			0, 0,loggrd);
	fprintf(stderr, "... done. " );
	...
	fprintf(stderr,"\n start of gzphase1 =%d ",  RUP_GRD(pbegall(&gzphase1,0)));
	fprintf(stderr,"\n end of gzphase1 = %d",  RUP_GRD(pendall(&gzphase1,0)));
	fprintf(stderr, "\n waitloc=%d, readpos = %d, \n pw_gz180crush2= %d pw_gzrf2= %d, pw_gzphase1= %d", 
		waitloc, readpos , pw_gz180crush2, pw_gzrf2, pw_gzphase1);
	fprintf(stderr, "\n pw_gx =%d , pw_gy = %d", pw_gx, pw_gy);
	fprintf(stderr, "\n t_tipdown_core =%d, t_readout_core =%d, \n seqtr =%d, optr=  %d \n", t_tipdown_core, t_readout_core,  seqtr,optr);
	fprintf(stderr,"\n RF2 starts at : %d",  RUP_GRD(pbegall(&rf2, 0) ));
	fprintf(stderr,"\n rewinder starts at : %d",  RUP_GRD(pbegall(&gxpre, 0) ));
	fprintf(stderr,"\n rewinder end at at : %d",  RUP_GRD(pendall(&gxpre, 0) ));

	SEQLENGTH(readout_core, t_readout_core - timessi + textra , readout_core);
	getperiod(&deadtime_readout_core, &readout_core, 0);
	...
	TRAPEZOID(ZGRAD, gzBS0rfspoiler, RUP_GRD(pendall(&BS0rf,0) +  pw_gzBS0rfspoilera),0,0,loggrd);
	...
	t_adjust =  RUP_RF(optr - t_preBS - t_tag - t_delay - seqtr );
	fprintf(stderr, "\npulsegen: Creating TRdelay adjust core ... ");
	fprintf(stderr,"\n optr: %d, t_preBS: %d, t_tag: %d,  t_delay: %d,  seqtr: %d, opslquant: %d, t_adjust:  %d", 
			optr, t_preBS, t_tag, t_delay, seqtr, opslquant,t_adjust );
...
#define RESERVED_IPG_MEMORY (0xbfff00)
	...
	switch (doRotated90){
		...
		case 2: /* rotation about the y axis of 90 degreees: x gradient */
		...
		case 1:/* rotation about the x axis of 90 degreees: y gradient */
			rotmatx[0][0] = 32767;
			rotmatx[0][1] = 0;
			rotmatx[0][2] = 0;
			rotmatx[0][3] = 0;
	...
	if(pre)  {
		...
		for (iv = 0; iv < ndisdaq; iv++) {	/*  disdaqs  */
			...
			if (doRotated90){
				...
				setrotate(rotmatx,0);
				setfrequency(myxmitfreq - (int)(delta_y*a_gzrf1*GAMMA_H1/(2*PI)) , &rf1, 0); 
			}
			else
			{
...
void doast(int* trig, int isLabel)
{
	...
	if (oprtcgate==1) settrigger( iTrigType,  0);
...
*/
	...
	/*fprintf(stderr,"\ndebug: kz slice = %d , z phase encode amplitude = %f", slicen, zpeamp);*/
	...
	*bangn = *bangn%nbang;      /*  for mps2  */
	...
	if (scluster==1)  {
		...
		if(slicen < nerror)		/* spread the error ... */
			tmp = deadtime_readout_core - trerror/nerror;	/* ... while keeping the sign */


	setperiod(RUP_GRD(tmp), &readout_core, 0);
	...
	/* fov offset for receiver */

	setwave(thetrecintlp[leafn], &thetrec, 0);
	if(gtype==2) setwave(thetrecintlp2[leafn], &thetrec2, 0);

	/* reset offset and go for it */
	boffset(off_readout_core);
