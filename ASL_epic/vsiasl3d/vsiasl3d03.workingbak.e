...
int	vsi_controlmode = 5 with {1,,,VIS, "Control Mode: (1) flip the magnitude  (2) no RF  (3) negative vel target (4) flip grads (5) no VelSel grads", } ;  
...
double 	vsi_Gmax = 4.0; /* default gradient max aplitude in VSI pulse train */
double 	vsi_Gcontrol = 4.0 ; /* default gradient max aplitude in control pulse train */
...
/*LHG 10.2015 */
int	vsi_train_len = 13500;  /* number of points in the pulse.  
...
double 	BIR_Gmax = 4.0; /* default gradient max aplitude in VSI pulse train */
int	BIR_len = 3500;  /* number of points in the iArterial suppression BIR8 pulse. */ 
...
int	BS1_time = 300000; /* first delay between BS inversion pulses , default to 300 ms after tag*/
int 	BS2_time = 850000;  /* delay between second BS inversion pulse and imaging, default to 850 ms after the first pulse */
float	rfscalesech;	/* ratio of areas: autoprescan sinc to SECH pulse */
int	doBS = 1;	/* background suppression pulses */
int	doArtSup = 1;	/*arterial suppression pulses */
...
int cveval()
{
	...
	cvdesc(opuser22,"Arterial Suppression Pulse (0=NO  1=YES)");
	cvmin(opuser22,0);
	cvmax(opuser22, 1);
	cvdef(opuser22,0);
	opuser22 = 0;
...
int predownload()
{
	...
	/* in sme cases, we want a different phase in the control pulses */
	for (i=0; i<Npoints ; i++){
...
STATUS pulsegen(void)
{
	...
	fprintf(stderr, "\n ... rfctl1 done ." );
        ...
        INTWAVE(ZGRAD,
                        ...
                        vsi_train_len,
			vsi_train_len*4,
                        vsi_pulse_grad, /* these should be in DAC units */
                        1,
                        loggrd);
	...
	INTWAVE(RHO,
			ASrf_mag,
			RUP_GRD(t_delay - fatsattime - BIR_len*4 - 1ms + psd_rf_wait ), 
			(float)doArtSup*1.0 ,  
			BIR_len,
			BIR_len*4,
			BIR_mag ,  
        ...
        INTWAVE(ZGRAD,
                        ASrf_grad,
			RUP_GRD(t_delay - fatsattime - BIR_len*4 - 1ms  ), 
